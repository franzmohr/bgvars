...
#'
#' summary method for class \code{"ctryvecest"}.
#'
#' @param object an object of class \code{"ctryvecest"}, usually, a result of a call to \code{\link{draw_posterior}}.
...
#' are used.
#' @param x an object of class \code{"summary.ctryvecest"}, usually, a result of a call to
#' \code{\link{summary.ctryvecest}}.
#' @param digits the number of significant digits to use when printing.
#' @param ... further arguments passed to or from other methods.
#'
#' @return \code{summary.ctryvecest} returns a list of class \code{"summary.ctryvecest"},
...
#'
#' @export
summary.ctryvecest <- function(object, ci = .95, period = NULL, ...){
  ...
  k_det_r <- 0
  names_det_r <- object[["model"]][["deterministic"]][["restricted"]]
  if (!is.null(names_det_r) & r > 0) {
    k_det_r <- length(object[["model"]][["deterministic"]][["restricted"]]) 
  }
  k_det_ur <- 0
  names_det_ur <- object[["model"]][["deterministic"]][["unrestricted"]]
  if (!is.null(names_det_ur)) {
  ...
  x_names <- .get_regressor_names_vec(object)
  ...
  vars <- c("pi_domestic", "pi_foreign", "pi_global", "pi_deterministic",
            "gamma_domestic", "gamma_foreign", "gamma_global", "gamma_deterministic", "a0")
  for (i in vars) {
  ...
  class(result) <- c("summary.ctryvecest", "list")
  return(result)
}
