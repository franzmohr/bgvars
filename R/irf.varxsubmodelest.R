...
#' 
#' Computes the impulse response coefficients of an object of class 'varxsubmodelest' for
#' \code{n_ahead} steps.
#' 
...
#' 
#' @export
irf.varxsubmodelest <- function(x, impulse, response, n_ahead = 5, ci = .95, shock = 1,
  ...
  draws_a <- NULL
  if (p_domestic > 0) {
    pos_a <- 1:n_a_domestic
    if (tvp) {
      pos_a <- rep(pos_a, tt) + rep(0:(tt - 1), each = length(pos_a)) * n_z
    }
    draws_a[["coeffs"]] <- t(x[["posteriors"]][["a"]][["coeffs"]][, pos_a])
  }
  
  # Unmodelled variables ----
  draws_b <- NULL
  pos_b <- n_a_domestic + 1:(n_a_foreign + n_b)
  if (length(pos_b) > 0) {
    if (tvp) {
      pos_b <- rep(pos_b, tt) + rep(0:(tt - 1), each = length(pos_b)) * n_z
    }
    draws_b[["coeffs"]] <- t(x[["posteriors"]][["a"]][["coeffs"]][, pos_b])
  }
  
  # Deterministic terms ----
  draws_c <- NULL
  if (n_c > 0) {
    pos_c <- n_a_domestic + n_a_foreign + n_b + 1:n_c
    if (tvp) {
      pos_c <- rep(pos_c, tt) + rep(0:(tt - 1), each = length(pos_c)) * n_z
    }
    draws_c[["coeffs"]] <- t(x[["posteriors"]][["a"]][["coeffs"]][, pos_c])
  }
