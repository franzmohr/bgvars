...
#' variables, which do not correspond to own lags, relative to argument \code{kappa1}.
#' @param kappa3 a numeric specifying the size of the prior variance of non-deterministic exogenous
#' variables relative to argument \code{kappa1}. Default is \code{NULL}, which indicates that the formula
    ...
    if (!is.null(object[["data"]][["train"]][["x"]])) {
      ...
      if (sigma == "AR") { # Univariate AR
        ...
        if (p_endogen > 0 | !is.null(pos_det)) {
          for (i in 1:k) {
            if (p_endogen > 0) {
              pos <- c(i + k * ((1:p_endogen) - 1), pos_det) 
            } else {
              pos <- pos_det
            }
            y_temp <- matrix(y[i, ], 1)
        ...
        for (r in 1:(p_exogen + 1)) {
          for (l in 1:k) {
            for (j in 1:k_exogen) {
              ...
              if (is.null(kappa3)) {
                V[l, k * p_endogen + (r - 1) * k_exogen + j] <- kappa1 * kappa4 * s_endo[l]^2
              } else {
                V[l, k * p_endogen + (r - 1) * k_exogen + j] <- kappa1 * kappa3 / r^2 * s_endo[l]^2 / s_exo[j]^2 
              }
            }
          }
        } 
        ...
        for (r in 1:(s + 1)) {
          for (l in 1:k) {
            for (j in 1:m) {
              # Note that in the loop r starts at 1, so that this is equivalent to l + 1
              if (is.null(kappa3)) {
                V[l, pos_offset + (r - 1) * m + j] <- kappa1 * kappa4 * s_endo[l]^2
              } else {
                V[l, pos_offset + (r - 1) * m + j] <- kappa1 * kappa3 / r^2 * s_endo[l]^2 / s_exo[j]^2 
              }
            }
          }
        } 
