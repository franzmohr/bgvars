...
Rcpp::List bgvaralg(Rcpp::List object) {
  ...
  const arma::mat diag_ktt = arma::eye<arma::mat>(k_dom * tt, k_dom * tt);
  ...
  ///////////////////////////////////////////////////////////////////////
  // Priors & initial values - Domestic, foreign and global coefficients
  ///////////////////////////////////////////////////////////////////////
  ...
  arma::mat draws_det = arma::zeros<arma::mat>(n_det, iter);
  ...
  for (int draw = 0; draw < draws; draw++) {
    ...
    if (draw >= burnin) {
      ...
      if (n_dom > 0) {
        draws_dom.col(pos_draw) = arma::vectorise(a.rows(dom_pos_start, dom_pos_end));
        if (varsel) {
          draws_lambda_dom.col(pos_draw) = lambda_vec.subvec(dom_pos_start, dom_pos_end);
        }
      }
      if (n_for > 0) {
        draws_for.col(pos_draw) = arma::vectorise(a.rows(for_pos_start, for_pos_end));
        if (varsel) {
          draws_lambda_for.col(pos_draw) = lambda_vec.subvec(for_pos_start, for_pos_end);
        }
      }
      if (n_glo > 0) {
        draws_glo.col(pos_draw) = arma::vectorise(a.rows(glo_pos_start, glo_pos_end));
        if (varsel) {
          draws_lambda_glo.col(pos_draw) = lambda_vec.subvec(glo_pos_start, glo_pos_end);
        }
      }
      if (n_det > 0) {
        draws_det.col(pos_draw) = arma::vectorise(a.rows(det_pos_start, det_pos_end));
        if (varsel) {
