...
The sub-models can be estimated inside R. `add_posterior_loglik` adds the draws
of the log-likelihood, from which the selection criteria below are calculated.
...
Either route leads to the same place: a folder of sub-model files that hold
posterior draws. The export above was written after the sub-models were
estimated in R, so the rest of this vignette works on it directly.
...
![plot of chunk selection-criteria](figures/GVAR-selection-criteria-1.png)
...
![plot of chunk girf](figures/GVAR-girf-1.png)
...
![plot of chunk gfevd](figures/GVAR-gfevd-1.png)
