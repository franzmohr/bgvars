---
title: "Global Vector Error Correction Models"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Global Vector Error Correction Models}
...
```{r poil-into-us-model, message=FALSE, eval = TRUE}
...
To reproduce the results of Dees et al. (2007), each sub-model is added manually for each country based on the specifications provided in the article's supplementary material.

```{r}
submodel <- "US"
object[["submodels"]][[submodel]] <- create_vecxsubmodel(object,
  submodel = submodel,
  endogen = c("y", "Dp", "rs", "rl", "q", "poil"),
  p_endogen = 2,
  exogen = c("y", "Dp", "ep"),
  p_exogen = 2,
  r = 2,
...
The resulting sub-models have different sample sizes. Function `align_model_obs` can be employed to ensure that all sub-models use the same set of observations that are common to all sub-models.
...
Since the function creates initial values based on the reduced rank maximum likelihood estimator, closer inspection of the sub-models should show that the produced estimates are equal to the values provided in the supplementary material to Dees et al. (2007).
...
# write_to_hdf5(object, folder = "D:/models/gvec-test")
# ```
